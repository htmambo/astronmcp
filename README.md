...
基于 [MCP 协议](https://modelcontextprotocol.io) 封装的多厂商 **Coding Plan** 服务，目前已支持讯飞星火 / 讯飞星辰 Coding Plan、火山方舟 Coding Plan 个人版与百度智能云千帆 Coding Plan，并为 OpenCode Go（experimental）提供 OpenAI 兼容子集接入，为 Claude Code 等 AI 编程工具提供代码审查、计划审查与多轮对话能力。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `PROVIDER` | `xfyun-coding` | `xfyun-coding` / `xfyun-http` / `xfyun-websocket` / `volcengine-coding` / `qianfan-coding` / `opencode-go` |
